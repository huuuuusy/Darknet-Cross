...
#define CL_DEVICE_OPENCL_C_VERSION 200   
#endif
...
void CLSetup::getPlatformID()
{
    ...
    std::cout<<"CL_COMPUTE PLATFORMS: "<<num_of_platforms<<std::endl;
    ...
    for (int i = 0; i <1; i++)
    {
        ...
        size_t platform_name_length = 1024;
	...
	std::cout<<"CL_COMPUTE PLATFORMS_NAME: "<<platform_name<<std::endl;
        // decide if this i-th platform is what we are looking for
        // we select the first one matched skipping the next one if any
        if (strstr(platform_name, /*"AMD"*/ "QUALCOMM Snapdragon(TM)" /*"Intel(R) OpenCL"*/) &&
...
void CLSetup::getDeviceID(char *devName)
{
    ...
    _status = clGetDeviceIDs(_platformID,CL_DEVICE_TYPE_GPU, 0, NULL, &_numDevices);
    //DEBUG_CL(_status);
    std::cout<<"CL_COMPUTE DEVICES: "<<_numDevices<<std::endl;
    _status = clGetDeviceIDs(_platformID, CL_DEVICE_TYPE_GPU, _numDevices, &_deviceID, NULL);
    //DEBUG_CL(_status);
