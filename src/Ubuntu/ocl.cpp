...
void ocl_pull_array(cl_mem x_gpu, float *x, size_t n)
{
	...
	status = clEnqueueReadBuffer(*m_OCLManager->m_OpenCLSetup.getQueue(), x_gpu, CL_TRUE, 0, size, x, 0, NULL, NULL);
	if (status != CL_SUCCESS) {
        printf("\n clRead buffer error : %s", getCLErrorString(status));
        exit(-1);
    }
}
// -------------------------------------------------------------------------------- Debug by Hu Shiyu 2 -------------------------------------------------
...
void forward_region_layer_opencl(const layer l, network_state state)
{
    ...
    m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_FLATARR]->pGlobal(ocl_blocks(l.outputs))->pLocal(OCL_BLOCK);
...
void shortcut_cpu(int batch, int w1, int h1, int c1, float *add, int w2, int h2, int c2, float *out)
{
   ...
   //ocl_push_array(l.output_ocl,l.output,l.outputs*l.batch);
}
...
// shortcut_layer.c
void forward_shortcut_layer_opencl(const layer l, network_state state)
{
    ocl_pull_array(state.input_ocl,state.input,l.outputs*l.batch);
...
void forward_upsample_layer_opencl(const layer l, network_state state)
{
//printf("\n UPSAMPLE \n");



    m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_FILL]->pGlobal(ocl_blocks(l.outputs))->pLocal(OCL_BLOCK);    
    m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_FILL]->arg(0, l.outputs*l.batch);
    m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_FILL]->arg(1, 0);
    m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_FILL]->arg(2, l.output_ocl);
    m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_FILL]->arg(3, 1);
    m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_FILL]->run(PROFILE_KERNELS, BLOCK_KERNEL_EXEC);

    if(l.reverse){
	m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->pGlobal(ocl_blocks(l.outputs))->pLocal(OCL_BLOCK);    
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(0, l.output_ocl);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(1, l.out_w);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(2, l.out_h);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(3, l.c);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(4, l.batch);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(5, l.stride);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(6, 0);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(7, l.scale);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(8, state.input_ocl);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->run(PROFILE_KERNELS, BLOCK_KERNEL_EXEC);
    }else{
	m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->pGlobal(ocl_blocks(l.outputs))->pLocal(OCL_BLOCK);    
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(0, state.input_ocl);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(1, l.w);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(2, l.h);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(3, l.c);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(4, l.batch);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(5, l.stride);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(6, 1);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(7, l.scale);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->arg(8, l.output_ocl);
        m_OCLManager->m_OpenCLKernels[NN_KERNEL_IDX_UPSAMPLE]->run(PROFILE_KERNELS, BLOCK_KERNEL_EXEC);
    }
}
...
void forward_yolo_layer_opencl(const layer l, network_state state)
{
    int i, j, b, t, n;
    ocl_pull_array(state.input_ocl,state.input,l.outputs*l.batch);
    ...
    //memcpy(l.output_ocl, l.output, l.outputs*l.batch * sizeof(float));
...
// upsample_layer.c
void forward_upsample_layer_cpu(const layer l, network_state state)
{
    //ocl_pull_array(state.input_ocl,state.input,l.outputs*l.batch);
...
// -------------------------------------------------------------------------------- Debug by Hu Shiyu 1 -------------------------------------------------
            ...
            forward_shortcut_layer_opencl(l, state);
            //printf("\n SHORTCUT \n");
        }
        else if (l.type == YOLO) {
            forward_yolo_layer_opencl(l, state);
            //printf("\n YOLO \n");
        }
// -------------------------------------------------------------------------------- Debug by Hu Shiyu 2 -------------------------------------------------
