...
// -------------------------------------------------------------------------------- Debug by Hu Shiyu 1 -------------------------------------------------
    ...
    FILE *fp;
    fp=fopen("multi_image_result.txt","w");
...
#endif
#endif
	float pred_time = (float)(clock() - time) / CLOCKS_PER_SEC;
        printf("%s: Predicted in %f seconds.\n", input, pred_time); //sec(clock() - time));
...
// -------------------------------------------------------------------------------- Debug by Hu Shiyu 1 -------------------------------------------------
	...
	sum_time =  sum_time + pred_time;
	if (i == 1) {
		ave_time = sum_time / i;
		printf("Total Image: %d, Sum Time: %f, Average Time: %f seconds.\n", i, sum_time, ave_time); 
		sum_time = 0;
		fprintf(fp,"image_num\tpred_time\tsum_time\tave_time\n");
	}
	else{
		ave_time = sum_time / (i - 1);
		printf("Total Image: %d, Sum Time: %f, Average Time: %f seconds.\n", i, sum_time, ave_time); 
	}
	char NAME [256];
	//n = n + 1;
        sprintf(NUM, "%d", n);
    	strcpy (NAME,"test3/prediction_");
        strcat (NAME,NUM);
        save_image_png(im, NAME);    // image.c
        show_image(im, NAME);        // image.c

	fprintf(fp,"%d\t%f\t%f\t%f\n", i, pred_time, sum_time, ave_time);
	i = i + 1;
// -------------------------------------------------------------------------------- Debug by Hu Shiyu 2 -------------------------------------------------
...
// -------------------------------------------------------------------------------- Debug by Hu Shiyu 1 -------------------------------------------------
	    float pred_time = after - before;
	    float curr = 1. / pred_time;
            fps = curr;
	    fprintf(fp,"%d\t%f\t%f\n", count, fps, pred_time);
// -------------------------------------------------------------------------------- Debug by Hu Shiyu 2 -------------------------------------------------
            before = after;
