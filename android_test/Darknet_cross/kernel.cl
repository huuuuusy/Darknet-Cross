...
__kernel void fill(int N, float ALPHA, __global float *X, int INCX)
{
...
__kernel void copy_gpu(int N, __global float *X, int OFFX, int INCX, __global float *Y, int OFFY, int INCY)
{
    int i = get_global_id(2) * get_global_size(0) * get_global_size(1) +
		get_global_id(1) * get_global_size(0) + get_global_id(0);
...
//only for LOGISTIC now
__kernel void activate_array_kernel(__global float *x, int n)
{
    int i = get_global_id(2) * get_global_size(0) * get_global_size(1) +
		get_global_id(1) * get_global_size(0) + get_global_id(0);
    if(i < n) 1.0F / (1.0F + exp(-x[i]));
}
